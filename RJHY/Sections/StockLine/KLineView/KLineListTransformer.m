...
/*
 ...
 Volume = 566477;
 */
NSString *const KKeyClose = @"Close";
NSString *const KKeyDate = @"Date";
NSString *const KKeyHigh = @"High";
NSString *const KKeyLow = @"Low";
NSString *const KKeyVolume = @"Volume";
NSString *const KKeyOpen = @"Open";
...
- (id)managerTransformData:(NSArray*)data {
    ...
    for (int i = (int)(data.count - 1); i > -1; i --) {
        //arr = @["日期,开盘价,最高价,最低价,收盘价,成交量, 调整收盘价"]wq
        NSDictionary *dic = [data objectAtIndexCheck:i];
        NSArray *arr = @[dic[KKeyDate],dic[KKeyOpen],dic[KKeyHigh],dic[KKeyLow],dic[KKeyClose],dic[KKeyVolume]];
/*
 Close = "3.82";
 Date = "2018-07-30";
 High = "4.18";
...
- (NSMutableArray*)chartMACDWithData:(NSArray *)data inRange:(NSRange)range {
    ...
    if (data.count - range.location > range.length) {
        ...
        if (range.location == data.count-2) {
            _EMA12 = [rangeData[0][KKeyClose] floatValue];
            _EMA26 = [rangeData[0][KKeyClose] floatValue];
            _DEA = 0;
            

        }else
        {
            CGFloat close = [rangeData[0][KKeyClose] doubleValue];
...
- (NSMutableArray*)chartKDJWithData:(NSArray *)data inRange:(NSRange)range {
    ...
    if (data.count - range.location > range.length) {
        ...
        for (NSDictionary  *dic in rangeData) {
            [highArr addObject:dic[KKeyHigh]];
            [lowArr addObject:dic[KKeyLow]];
        }
        ...
        CGFloat RSV = ([rangeData[0][KKeyClose] floatValue] - minClose)*100/(maxClose-minClose);
...
- (CGFloat)chartRSIWithData:(NSArray *)data inRange:(NSRange)range {
    ...
    NSArray *rangeData = [data objectsAtIndexes:[NSIndexSet indexSetWithIndexesInRange:range]];
        ...
        for (NSDictionary *item in rangeData) {
            ...
            [closePriceArr addObject:[item objectForKey:KKeyClose]];
        }
...
- (CGFloat)chartMAWithData:(NSArray *)data inRange:(NSRange)range {
    ...
    if (data.count - range.location > range.length) {
         NSArray *rangeData = [data objectsAtIndexes:[NSIndexSet indexSetWithIndexesInRange:range]];
        for (NSDictionary *item in rangeData) {
            md += [[item objectForKey:KKeyClose] floatValue];
        }
